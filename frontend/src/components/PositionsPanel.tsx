...
function PositionRow({ pos, prices, trader }: { pos: Position; prices: Record<string, PriceData>; trader: `0x${string}` }) {
  const { wallets } = useWallets();
  const [closing, setClosing] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const pairLabel = Object.keys(PYTH_IDS).find((k) => k.startsWith(pos.pair.slice(0, 3))) ?? pos.pair;
  const currentPriceData = prices[pairLabel];
  ...
  async function handleClose() {
    ...
    try {
      ...
      const feedId = PYTH_IDS[pairLabel];
      if (!feedId) throw new Error("No Pyth feed");
      ...
      setError(err instanceof Error ? err.message.slice(0, 60) : "Close failed");
    } finally {
      setClosing(false);
    }
  }
    ...
    <div style={rowStyles.row}>
      <div style={rowStyles.left}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 600 }}>{pairLabel}</span>
        </div>
        <div style={rowStyles.meta}>
          <span className="label">Entry</span>
          <span className="price" style={{ fontSize: 11 }}>${(Number(pos.entryPrice) / 1e8).toLocaleString("en-US", { minimumFractionDigits: 2 })}</span>
          <span className="label" style={{ marginLeft: 8 }}>Notional</span>
          <span className="price" style={{ fontSize: 11 }}>${(Number(pos.notional) / 1e6).toLocaleString("en-US", { maximumFractionDigits: 0 })}</span>
        </div>
        <div style={rowStyles.health}>
      ...
      <div style={rowStyles.right}>
        <span style={{ fontSize: 14, fontWeight: 700, color: pnl >= 0 ? "var(--green)" : "var(--red)", fontFamily: "var(--font-price)" }}>
          {pnl >= 0 ? "+" : ""}${pnl.toFixed(2)}
        </span>
        <button
...
export function PositionsPanel({ trader, prices }: Props) {
  ...
  const { positions, loading } = usePositions(trader);
    ...
    <div style={styles.wrapper}>
      ...
      {!authenticated ? (
        ...
        <div style={styles.list}>
          {positions.map((pos) => (
            <PositionRow key={pos.id} pos={pos} prices={prices} trader={trader!} />
          ))}
        </div>
      )}
    </div>
...
const styles = {
  ...
  empty: { padding: "24px 16px", color: "var(--text-secondary)", fontSize: 12, textAlign: "center" as const },
...
const rowStyles = {
  ...
  left: { display: "flex", flexDirection: "column" as const, gap: 4, flex: 1, minWidth: 0 },
  meta: { display: "flex", alignItems: "center", gap: 4 },
  health: { display: "flex", alignItems: "center", gap: 6, maxWidth: 180 },
  right: { display: "flex", flexDirection: "column" as const, alignItems: "flex-end", flexShrink: 0 },
} satisfies Record<string, React.CSSProperties>;
